...
export function useUserManagement() {
  ...
  useEffect(() => {
    console.log(
      "🔄 useEffect [authLoading, token, searchKeyword, isUsingFilter, selectedRole] triggered"
    );
    loadUsers();
  ...
  useEffect(() => {
    ...
    if (totalCount >= 0 && pageSize > 0) {
      ...
      if (page > lastPageIndex) {
        console.log(
          "  ⚠️ Page out of bounds! Resetting to lastPageIndex:",
          lastPageIndex
        );
  ...
  const buildFilterQuery = (keyword: string, roleFilter: number | null) => {
    ...
    if (keyword) {
      ...
      filters.push(
        `(contains(tolower(fullName),'${safe}') or contains(tolower(email),'${safe}') or contains(tolower(phone), '${safe}') or contains(tolower(address), '${safe}'))`
      );
    }
  ...
  const handleOpenCreateDialog = () =>
    setDialogStates({ ...dialogStates, create: true });
