...
class ApiClient {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401 && responseData.requireRelogin === true) {
          console.warn('🚨 Browser fingerprint mismatch detected:', responseData.message);
          this.handleBrowserFingerprintMismatch(responseData);
          throw new BrowserFingerprintMismatchError(responseData.message || 'Browser fingerprint mismatch');
        }
  ...
  private handleBrowserFingerprintMismatch(errorData: any) {
    ...
    toast.error('🔒 Security Alert: Browser fingerprint mismatch. Redirecting to login...', {
      duration: 4000,
    });
